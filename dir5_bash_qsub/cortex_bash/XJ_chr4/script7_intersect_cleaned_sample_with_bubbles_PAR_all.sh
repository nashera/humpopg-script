...
cleaned_graph_colourlist="${pool_chr_all}/$pop.chr$chr.k$kmer.q$quality.cleaned_bin.for_samples_intersection.colourlist"
...
echo "$ctx_binary --kmer_size $kmer --mem_height $mem_height --mem_width $mem_width --multicolour $bubble_graph --colour_list ${cleaned_graph_colourlist} --load_colours_only_where_overlap_clean_colour 0 --successively_dump_cleaned_colours $suffix > $log 2>&1"
