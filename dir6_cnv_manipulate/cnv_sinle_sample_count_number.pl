...
my $inp_dir="/picb/papgi-pgg/zhangxi/ngs_CNV_analysis/breakdancer_dir_analysis/single_cnv_dir";
my $out_dir="/picb/papgi-pgg/zhangxi/ngs_CNV_analysis/breakdancer_dir_analysis/single_cnv_dir";
opendir(DIRHANDLE,"$inp_dir") || die $!;
my @samples=grep{/^WGC/} readdir DIRHANDLE;
...
my @inv_array;
my @ins_array;
for my $i(0..$#sorted_samples){
	my $single_del_number=0;
	my $single_itx_number=0;
	my $single_inv_number=0;
	my $single_ins_number=0;
	my @single_chr_length=();
	my $line=0;
	open (my $single_file,"<","$inp_dir/$sorted_samples[$i]") || die $!;
	while (<$single_file>){
		$line++;
		chomp;
		my @entry=split;
	...
	push @cnv_del_number,$single_del_number;
	push @cnv_itx_number,$single_itx_number;
	push @cnv_inv_number,$single_inv_number;
	push @cnv_ins_number,$single_ins_number;
	print "samples $i+1 complete\n";
...
open (my $file_single_cnv_number,">","$out_dir/single_sample_cnv_sample.txt");
print $file_single_cnv_number "IND\tDEL\tITX\tINV\tINS\n";
for my $i(0..$#sorted_samples){
	my $ind=substr($sorted_samples[$i],0,10);
	print $file_single_cnv_number "$ind\t$cnv_del_number[$i]\t$cnv_itx_number[$i]\t$cnv_inv_number[$i]\t$cnv_ins_number[$i]\n";
	}

print $file_single_cnv_number "total\t$del_total_number\t$itx_total_number\t$inv_total_number\t$ins_total_number\t$total\n";
