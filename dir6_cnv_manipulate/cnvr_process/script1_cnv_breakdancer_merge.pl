...
my $inp_dir="/picb/papgi-pgg/zhangxi/xinjiang_ngs_analysis/xinjiang_ngs_sortedBAM_chr_breakdancer";
my $out_dir="/picb/papgi-pgg/zhangxi/xinjiang_ngs_analysis/ngs_CNV_analysis/breakdancer_dir_analysis/cnvr_analysis";
...
for my $i(0..$#all_ind){
	...
	for my $j(0..$#all_chr){
		...
		my $length=scalar(@$ra_ind_chr_sv_output)-1;
		if(! -e $y){
		mkdir("$out_dir/$all_ind[$i]/chr_$chr",0777) || die $!;
	}
		chdir "$out_dir/$all_ind[$i]/chr_$chr" || die $!;
		for my $k(0..$length){
			my $out_cnv_type_file="$y/$all_ind[$i]\_chr$chr\_$ra_ind_chr_sv_output->[$k][4]";
			if (-e $out_cnv_type_file){
				open (OUT,">>","$out_cnv_type_file") || die $!;}
			else{
				open (OUT,">","$out_cnv_type_file") || die $!;}
			print OUT "$ra_ind_chr_sv_output->[$k][0]\t$ra_ind_chr_sv_output->[$k][1]\t$ra_ind_chr_sv_output->[$k][2]\t$ra_ind_chr_sv_output->[$k][3]\t$ra_ind_chr_sv_output->[$k][4]\n";
		}
