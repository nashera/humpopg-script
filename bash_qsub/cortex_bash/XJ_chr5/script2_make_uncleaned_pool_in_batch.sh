...
chr=5
process="/home/zhangxi/Document/LOG/chr${chr}_batch_cortex_make_unclean_bin_q20.txt"
if [ -e "$process" ];then
	rm $process
fi
...
if [ ! -e "$pool/XJ_chr$chr/all" ];then
	mkdir -p $pool/XJ_chr$chr/all
fi
kmer=31
height=25
width=150
...
for((i=1;i<=4;i++));do
	...
	if [ -e "${batch_merged_pool_bin}" ];then
		continue
	fi
